...
fn material_shadow(
    ...
    let core = clamp(screen_px_range * (sd - 0.5) + 0.5, 0.0, 1.0);
    let core_a = clamp(screen_px_range * (sd_alpha - 0.5) + 0.5, 0.0, 1.0);
    let ink = mix(core, core_a, smoothstep(0.04, 0.12, spread));
    let dilated = clamp(screen_px_range * (sd - 0.5) + 0.5 + blur, 0.0, 1.0);
    let dil_a = clamp(screen_px_range * (sd_alpha - 0.5) + 0.5 + blur, 0.0, 1.0);
